<?php
	...
	$tab3 = array();
	//$tab4 = array();
    $requete=$bd->Select('SELECT f.id id_file, lat, lon, url, geon, feat, description FROM files f left outer join metadata m on f.id= m.id_file ORDER BY RAND() LIMIT 12');//SELECT f.id id_file, lat, lon, url, geon, feat, description FROM files f left outer join metadata m on f.id= m.id_file where f.url="http://eol.jsc.nasa.gov/sseop/images/ISD/lowres/AS06/AS06-2-986.JPG" LIMIT 12');
